  ...
  class HTMLCanvas {
    ...
    hashChanged() {
      if (window.clearedHash != window.location.hash) {
        ...
        if (currentTarget) {
          ...
          console.log('remove targethack', currentTarget.classList);
        }
        if (window.location.hash) {
          var newTarget = document.querySelector(window.location.hash);
          if (newTarget) {
            newTarget.classList.add('targethack');
            console.log('add targethack', newTarget.classList);
          }
        }
      }
      ...
      if (ele != this.lastEle) {
        if (ele) {
          ...
          do {
            ...
            if (this.overElements.indexOf(current) == -1) {
              if (current.classList) {
                  current.classList.add("hoverhack");
                  console.log('add hoverhack', current.classList);
              }
          ...
          for (var i = 0; i < this.overElements.length; i++) {
            ...
            if (parents.indexOf(element) == -1) {
              if (element.classList) {
                  element.classList.remove("hoverhack");
                  console.log('remove hoverhack #1', element.classList);
              }
          ...
          while (element = this.overElements.pop()) {
            if (element.classList) {
                element.classList.remove("hoverhack");
                console.log('remove hoverhack #2', element.classList);
            }
    ...
    mousedown(x, y, button) {
      ...
      if (ele) {
        ...
        console.log('adding activehack, removing hoverhack', ele.classList);
    ...
    setFocus(ele) {
      ...
      console.log('add focushack', ele.classList);
    ...
    setBlur() {
      if (this.focusElement) {
        ...
        console.log('remove focushack', this.focusElement.classList);
    ...
    clearHover() {
      ...
      while (element = this.overElements.pop()) {
        if (element.classList) {
            element.classList.remove("hoverhack");
            console.log('remove hoverhack', element.classList);
        }
      ...
      if (activeElement) {
        ...
        console.log('remove activehack', activeElement.classList);
    ...
    mouseup(x, y, button) {
      ...
      if (this.activeElement) {
        ...
        console.log('remove activehack', this.activeElement.classList);
        if(ele){
          ele.classList.add("hoverhack");
          console.log('add hoverhack', ele.classList);
